...
$iScriptId = cssLogStart( "fraudPhoneAlert.php" );
...
$configSql = "SELECT alertEmailName, fraudTriggerGroupSize, alertTriggerPercent, enabledStatus 
		FROM alertEmails 
		WHERE alertEmailName='fraudPhoneAlert'";
...
$phoneFlagged = 0;
$phoneSourceFlagged = array();

// locate all potentially fraudulent phones
$sPhoneAllQuery = "SELECT phoneNo, count(*) as count
...
// Count total leads with (p)fraud phone
// Count subtotals broken down by sourcecode
$rPhoneAllResult = dbQuery($sPhoneAllQuery);
while( $oAllRow = dbFetchObject( $rPhoneAllResult ) ) {

	// For each phone, find the associated email in userDataHistory
	...
	while( $oSourceRow = dbFetchObject( $rLeadSourceResult )) {
		...
		$phoneSourceFlagged[$oSourceRow->sourceCode][flagged] += $oSourceRow->count;
		$phoneSourceFlagged[$oSourceRow->sourceCode][subSourceCodes][$oSourceRow->subSourceCode] += $oSourceRow->count;
		// keep a running total of flagged users across all source codes
		$phoneFlagged += $oSourceRow->count;
	}
}
...
$sInternalReport .= "<tr><td class=big>Total leads with fraudulent Phone characteristics:  $phoneFlagged / $grandTotal = " . 100 * number_format( $phoneFlagged / $grandTotal, 4 ). "</td></tr>";
$sInternalReport .= "<tr><td class=big>Source codes flagged due to high percentage of leads with like Phone:</td></tr></table><br><br>";

// sort by source code
$sortedPhoneSourceFlagged = ksort($phoneSourceFlagged);
...
foreach( $phoneSourceFlagged as $source => $sourceData ) {
...
if( strtolower($oConfigRow->enabledStatus) == 'c' || strtolower($oConfigRow->enabledStatus) == 'b') {
	foreach( $clientReports as $id => $clientReport ) {
		...
		if( $clientReport[emails] ) {
			...
			$sReportTitle = $clientReport[companyName] . " Potentially Fraudulent Phone Stats " . $sToday;
...
First, it runs query # 1 and get list of phone numbers.  Then for
each phone number, it runs query # 2 to find out how many 
leads were collected with same phone number by source code.
...
$sInternalReport = $sInternalReport."</table><br><br><table>
		...
		<tr><td class=small colspan=3>Fraud: Total leads with fraudulent Phone characteristics per source code.</td></tr>
...
if( strtolower($oConfigRow->enabledStatus) == 'i' || strtolower($oConfigRow->enabledStatus) == 'b') {
	...
	$sSubject = "ALL PARTNERS Potentially Fraudulent Phone Stats " . $sToday;
