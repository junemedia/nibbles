...
$iScriptId = cssLogStart( "fraudIpAlert.php" );
...
$configSql = "SELECT alertEmailName, fraudTriggerGroupSize, alertTriggerPercent, enabledStatus 
		FROM alertEmails WHERE alertEmailName='fraudIpAlert'";
...
$ipFlagged = 0;
$ipSourceFlagged = array();

// locate all potentially fraudulent ips
$sIpAllQuery = "SELECT remoteIp, count(*) as count
		FROM otDataHistory
		WHERE dateTimeAdded BETWEEN '$sFrom 00:00:00' AND '$sTo 23:59:59'
		GROUP BY remoteIp
...
// Count total leads with (p)fraud ip
// Count subtotals broken down by sourcecode
$rIpAllResult = dbQuery($sIpAllQuery);
while( $oAllRow = dbFetchObject( $rIpAllResult ) ) {
	// keep a running total of flagged users across all source codes
	$ipFlagged += $oAllRow->count;
	
	// For each ip, group by source code and subSourceCode and count the number of times the ip appears
	...
	while( $oSubSourceRow = dbFetchObject( $rLeadSubSourceResult )) {
		...
		$ipSourceFlagged[$oSubSourceRow->sourceCode][flagged] += $oSubSourceRow->count;
		$ipSourceFlagged[$oSubSourceRow->sourceCode][subSourceCodes][$oSubSourceRow->subSourceCode] += $oSubSourceRow->count;
	}
}
...
$sInternalReport .= "<tr><td class=big>Total leads with fraudulent IP address characteristics:  $ipFlagged / $grandTotal = " . 100 * number_format( $ipFlagged / $grandTotal, 4 ). "</td></tr>";
$sInternalReport .= "<tr><td class=big>Source codes flagged due to high percentage of leads with like IP Address:</td></tr></table><br><br>";

// sort by source code
$sortedIpSourceFlagged = ksort($ipSourceFlagged);
...
//From here, we want to sort the whole thing by percentage of fraud
//======begin new sort
$ipReportSorter = array();
$sourceDatas = array();
foreach($ipSourceFlagged as $source => $sourceData){
	$ipReportSorter[$source] = 100 * number_format( $sourceData[flagged] / $sourceCodeTotals[$source], 4 );
	$sourceDatas[$source] = $sourceData;
}

array_multisort($ipReportSorter, SORT_DESC);
//print_r($ipReportSorter);
//======end new sort

foreach($ipReportSorter as $source => $fraudPct){
//foreach( $ipSourceFlagged as $source => $sourceData ) {
	$sourceData = $sourceDatas[$source];
...
if( strtolower($oConfigRow->enabledStatus) == 'c' || strtolower($oConfigRow->enabledStatus) == 'b') {
	foreach( $clientReports as $id => $clientReport ) {
		...
		if( $clientReport[emails] ) {
			...
			$sReportTitle = $clientReport[companyName] . " Potentially Fraudulent IP Address Stats " . $sToday;
...
First, it runs query # 1 and get list of remoteIp addresses.  Then for
each remoteIp address, it runs query # 2 to find out how many 
leads were collected with remoteIp address by source code.
...
<br><br>
<b>Query 1:</b>
SELECT remoteIp, count(*) as count
FROM otDataHistory
WHERE dateTimeAdded BETWEEN '$sFrom 00:00:00' AND '$sTo 23:59:59'
GROUP BY remoteIp
HAVING count >= $oConfigRow->fraudTriggerGroupSize 
...
FROM otDataHistory
WHERE dateTimeAdded BETWEEN '$sFrom 00:00:00' AND '$sTo 23:59:59'
AND remoteIp = [IP ADDRESS]
GROUP BY sourceCode, subSourceCode
<td></tr>";

$sInternalReport = $sInternalReport."</table><br><br><table>
		<tr><td class=small colspan=3><b>Notes:</b><br></td></tr>
		<tr><td class=small colspan=3>Fraud: Total leads with fraudulent IP characteristics per source code.</td></tr>
...
if( strtolower($oConfigRow->enabledStatus) == 'i' || strtolower($oConfigRow->enabledStatus) == 'b') {
	...
	$sSubject = "ALL PARTNERS Potentially Fraudulent IP Address Stats " . $sToday;
