...
if ($NUM_BACKUPS <> 0) {
	...
	echo "Delete Oldest Backup Started: ".date('Y-m-d H:i:s')."\n";
	...
	for ($VERSION=$NUM_BACKUPS; $VERSION > 1; $VERSION--) {
		...
		if ( is_dir($SECONDARY_BACKUP_DIR."/daily.".$SEND_VERSION) ) {
			...
			if ( rename ($SECONDARY_BACKUP_DIR."/daily.".$SEND_VERSION, $SECONDARY_BACKUP_DIR."/daily.".$VERSION) ) {
				...
				echo "Moving ".$SECONDARY_BACKUP_DIR."/daily.".$SEND_VERSION." to ".$SECONDARY_BACKUP_DIR."/daily.".$VERSION."\n";
	...
	echo "Shift Backups Finished, duration: $tempDateDuration seconds\n\n";

	# rename current to daily.1, if it exists
	$tempDateStart = date('U');
	$outputlog .= "Rename Backup Started: ".date('Y-m-d H:i:s')."\n";
	echo "Rename Backup Started: ".date('Y-m-d H:i:s')."\n";
	...
	if ( is_dir($MAIN_BACKUP_DIR."/current") ) {
		...
		if ( rename ($MAIN_BACKUP_DIR."/current", $SECONDARY_BACKUP_DIR."/daily.1") ) {
			...
			echo "Moving ".$MAIN_BACKUP_DIR."/current to ".$SECONDARY_BACKUP_DIR."/daily.1\n";
	...
	echo "Deleting ".$MAIN_BACKUP_DIR."/current\n";

	$tempDateEnd = date('U');
	$tempDateDuration = $tempDateEnd - $tempDateStart;
	$outputlog .= "Rename Backup Finished, duration: $tempDateDuration seconds\n\n";
	echo "Rename Backup Finished, duration: $tempDateDuration seconds\n\n";
...
if (($SECONDARY_BACKUP_DIR) && (!is_dir($SECONDARY_BACKUP_DIR))) {
	...
	echo "Creating ".$SECONDARY_BACKUP_DIR."\n\n\n";
}
...
echo "Backing up all databases on ".$MYSQLSERVER."...\n\n";

$tempDateStartDumps = date('U');
$outputlog .= "Dumps Started: ".date('Y-m-d H:i:s')."\n\n";

echo "Dumps Started: ".date('Y-m-d H:i:s')."\n\n";
...
if( $bDumps ) {
	...
	for ($k=0; $k < count($db_names); $k++) {
		...
		echo "Database '$db_names[$k]' Started: ".date('Y-m-d H:i:s')."\n";
		...
		echo "Creating ".$sBackupDumpsDir."/".$db_names[$k]."\n\n\n"."Backing up database ".$db_names[$k]."\n\n";
		...
		if ($result) {
			...
			for ($i=0; $i<count($tb_names); $i++) {
				...
				echo "Table '$db_names[$k].$tb_names[$i]' Started: ".date('Y-m-d H:i:s')."\n";
				...
				if ($result) {
					...
					echo " ERROR! - ".$db_names[$k]." Not Backed Up!\n";
				} else {
					...
					if ($bCompress) {
						...
						echo "Compressing ".$sBackupDumpsDir."/".$db_names[$k]."/".$tb_names[$i].".sql\n\n";
						...
						echo "Deleting file ".$sBackupDumpsDir."/".$db_names[$k]."/".$tb_names[$i].".sql\n";
					}
				}
				$tempDateEnd = date('U');
				...
				echo "Table '$db_names[$k].$tb_names[$i]' Finished, duration: $tempDateDuration seconds\n\n";
			...
			if ($result) {
				...
				echo " ERROR! - ".$db_names[$k]." Not Zipped!\n";
			}
			...
			echo "Database '$db_names[$k]' Finished, duration: $tempDateDuration seconds\n\n\n\n";
...
echo "Binaries Started: ".date('Y-m-d H:i:s')."\n\n";
		...
		if( is_dir( "$sBinaryDir/$sBinaryDirDb" ) && $sBinaryDirDb != "." && $sBinaryDirDb != ".." ) {
			...
			while(($sBinaryFile = readdir($hBinaryDbDir)) != false) {
				if( $sBinaryFile != "." && $sBinaryFile != ".." && substr($sBinaryFile, -3, 3 ) == "MYD" ) {
					...
					echo "Binary ($sBinaryDir/$sBinaryLoc) Started: ".date('Y-m-d H:i:s')."\n";
					...
					if( !$rLockTable ) {
						...
						echo "FAILURE: Did not lock table $sBinaryLoc!\n";
					} else {
						...
						if ( !$rFlushTable ) {
							...
							echo "FAILURE: Could not flush table $sBinaryLoc!\n";
							...
							if ( !$rUnlockTablesFail ) {
								...
								echo "FAILURE: Could not unlock table $sBinaryLoc!\n";
								...
								echo "Recovered.  Tables unlocked, but file not copied.";
							}
						} else {

							$outputlog .= "Creating $sBackupBinariesDir/$sBinaryLoc.\n";
							
							echo "Creating $sBackupBinariesDir/$sBinaryLoc.\n";
							...
							echo "Copy (with preserve) $sBinaryDir/$sBinaryDirDb/$sBinaryLoc.* $sBackupBinariesDir/$sBinaryDirDb\n";
							...
							echo "Binary ($sBinaryDir/$sBinaryLoc) Finished, duration: $tempDateDuration\n\n";
