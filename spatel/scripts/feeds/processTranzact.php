...
$rResult = mysql_query("TRUNCATE TABLE nibbles_temp.tranzactTemp");
...
while ($sData = mysql_fetch_object($rResult)) {
	...
	$sInsertQuery = "INSERT IGNORE INTO nibbles_temp.tranzactTemp (email,first,last,address,address2,city,state,zip,date,ip,src)
		VALUES (\"$sEmail\",\"$sFirst\",\"$sLast\",\"$sAddress\",\"$sAddress2\", 
		\"$sCity\",\"$sState\",\"$sZip\",\"$sDateTime\",\"$sRemoteIp\",\"$sSourceCode\")";
	$rInsertResult = mysql_query($sInsertQuery);
}

echo "\n\nStep2\n\n";

$sGetJoinEmailSubDataQuery = "SELECT * FROM joinEmailSub WHERE dateTimeAdded BETWEEN '$sFrom' AND '$sTo' AND email LIKE '%@aol.com'";
...
while ($sJoinEmailSubRow = mysql_fetch_object($rGetJoinEmailSubDataResult)) {
	...
	if (mysql_num_rows($rTempDataQueryResult) > 0) {
		...
		$sInsertQuery = "INSERT IGNORE INTO nibbles_temp.tranzactTemp (email,ip,date,sourceCode)
...
$sDelete = "DELETE FROM nibbles_temp.tranzactTemp WHERE ip=''";
$rDelete = mysql_query($sDelete);

$sDelete = "DELETE FROM nibbles_temp.tranzactTemp WHERE email NOT LIKE '%@aol.com'";
$rDelete = mysql_query($sDelete);
...
while ($rRow = mysql_fetch_object($rGetResult)) {
	$sDelete = "DELETE FROM nibbles_temp.tranzactTemp WHERE email LIKE '%$rRow->TLDs'";
	$rDelete = mysql_query($sDelete);
}
...
while ($rRow2 = mysql_fetch_object($rResult2)) {
	$sDelete = "DELETE FROM nibbles_temp.tranzactTemp WHERE email LIKE '%$rRow2->domain'";
	$rDelete = mysql_query($sDelete);
}
...
while ($rRow3 = mysql_fetch_object($rResult3)) {
	$sDelete = "DELETE FROM nibbles_temp.tranzactTemp WHERE email = '$rRow3->email'";
	$rDelete = mysql_query($sDelete);
}
...
while ($rRow4 = mysql_fetch_object($rResult4)) {
	$sDelete = "DELETE FROM nibbles_temp.tranzactTemp WHERE sourceCode = '$rRow4->sourceCode'";
	$rDelete = mysql_query($sDelete);
}
...
$iCount = 0;
$rFile = fopen("/home/tranzact/Ampere-".$sYesterday.".csv","w");
if ($rFile) {
	$sFinalDataQuery = "SELECT * FROM nibbles_temp.tranzactTemp";
...
$sFtp_User = "amperemedia";
$sFtp_Pass = "1cATEARN";
$sFtp_Server = "66.109.21.234";
...
if (!$sConnection_Id) {
	...
	mail('it@amperemedia.com', 'processTranzact FTP Failed', $sEmailMessage , "From: spatel@amperemedia.com\r\n");
} else {
	// upload a file
	if (ftp_put($sConnection_Id, "$sFile", "/home/tranzact/"."$sFile", FTP_ASCII)) {
