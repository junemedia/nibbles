...
$sQuery = "SELECT * FROM nibbles.otData WHERE offerCode = 'TSER_TGCL' AND howSent='rte'";
...
while ($sOtRow = mysql_fetch_object($sOtResult)) {
	$sXmlContent = "<?xml version='1.0' encoding='utf-8' standalone='no' ?>
		...
		<order>
		 <item>
		  <productName>Lawn Analysis</productName>
		  <option>
		   <name>LawnAnalysisType</name>
		   <value>14PointLawn</value>
		  </option>
		  <property>
		   <name>adName</name>
		  ...
		  <comments>Primary Interest: [field2]</comments>
		</item>
		</order>
		</wshRequest>";
	...
	$aPiece = explode('|',$sOtRow->page2Data);
	$sXmlContent = str_replace("[field2]",str_replace("\"",'',$aPiece[1]), $sXmlContent);
