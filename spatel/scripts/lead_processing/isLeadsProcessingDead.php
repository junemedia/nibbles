...
$liveAddresses = array('it@amperemedia.com','6306700018@messaging.sprintpcs.com');
$testAddresses = "spatel@amperemedia.com";
...
if(!in_array($scriptNameToLookFor, array_keys($vars))){
	foreach($liveAddresses as $eml){
		mail($eml, 'bad script argument.', $scriptNameToLookFor." isn't a proper script name.");
	}
	exit();
}
...
if($out){
	foreach($liveAddresses as $eml){
		mail($eml, "$scriptNameToLookFor may have problems (overnight)",$out);
	}
}
