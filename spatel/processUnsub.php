...
function Arcamax_Unsub_Request($post_string) {
	$server_response = '';
...
$x = 0;
$result = mysql_query("SELECT DISTINCT email FROM processUnsub");
while ($row = mysql_fetch_object($result)) {
	...
	while ($user_row = mysql_fetch_object($get_user_data_result)) {
		...
		$send_to_arcamax = Arcamax_Unsub_Request($post_string);
	...
	$delete_query = "DELETE FROM processUnsub WHERE email =\"$email\"";
	$delete_query_result = mysql_query($delete_query);
	echo mysql_error();
