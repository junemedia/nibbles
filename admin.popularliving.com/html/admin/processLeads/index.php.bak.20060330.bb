				...
				ob_flush();
					...
					while ($oOffersRow = dbFetchObject($rOffersResult)) {
						...
						if (($iTempLeadsGroupId != 0  && $iTempPrevGroupId != $iTempLeadsGroupId )) {
							...
							/*
							//these are suggested improvements here
							
							//so, the higher loop here is on leadsGroupId, so let's find the offer codes
							//(there are going to be multiple) for each one.
							$sOfferCodesGroupsQuery = "SELECT offerCode,maxAgeOfLeads,leadFileName FROM offerLeadSpec WHERE leadsGroupId = '$iTempLeadsGroupId'";
							$rOfferCodesGroupsResult = dbQuery($sOfferCodesGroupsQuery);
							$aOfferCodeEmails = array();
							$aOfferCodeFileNames = array();
							while($oOfferCode = dbFetchObject($rOfferCodesGroupsResult)){
								$aOfferEmails = array();
								$sEmailsGroupsQuery = "SELECT email
														FROM otDataHistory
														WHERE offerCode = $oOfferCode->offerCode
														AND processStatus = 'P' 
														AND verified != 'I' 
														AND sendStatus IS NULL 
														AND DATE_ADD(date_format(dateTimeAdded,"%Y-%m-%d"), INTERVAL $oOfferCode->maxAgeOfLeads DAY) >= CURRENT_DATE AND address NOT LIKE '3401 DUNDEE%'";
								
								$rEmails = dbQuery($sEmailsGroupsQuery);
								while($oEmail = dbFetchObject($rEmails)){
									if(!in_array($oEmail->email, $aOfferEmails))
										array_push($aOfferEmails, $oEmail->email);
								}

							//now we've got a bunch of emails, and we don't know if they're postal verified or not
							//so, let's use a string join and an IN, and userDataHistory will have these back to us very quickly
								$sGroupOffersCountQuery = "SELECT count(*) as count,
											FROM userDataHistory 
											WHERE email in ('".join("','",$aOfferEmails)."')
											AND postalVerified = 'V'";	
											
								$aOfferEmails = array();
								$rCounts = dbQuery($sGroupOffersCountQuery);
								$oCount = dbFetchObject($rEmails);
								
								//then, we just put all of those into an associative array that we can loop over
								$aOfferCodeEmails[$oOfferCode->offerCode] = $oCount->count;
								$aOfferCodeFileNames[$oOfferCode->offerCode] = $oOfferCode->leadFileName;
							}
							
							//and the while below can just be a foreach($aOffCodeEmails as $sOfferCode => $iEmailsCount)...
							*/
