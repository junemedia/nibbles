	...
	if (count($_SESSION['aSesPage2Offers']) == 0) {
		...
		if ($iPage2Info) {
			...
			if ($_SESSION["sSesRedirectUrl"] != '') {
				...
				echo "<script language=JavaScript>parent.location='http://www.popularliving.com/p/onetime.php?".SID."'</script>";
			}
		}
	}
...
if (count($_SESSION['aSesPage2Offers']) == 0) {
	...
	if (dbNumRows($rOffersResult) > 0) {
		while ($oOffersRow = dbFetchObject($rOffersResult)) {
			...
			$sIsRequireSSL = $oOffersRow->isRequireSSL;
...
if ($sIsRequireSSL == 'Y') {
	$sUrl = "https://".$_SERVER['SERVER_NAME']."/nibbles2/preview.php";
} else {
	$sUrl = "preview.php";
}
...
<form name=form1 action='$sUrl' method=POST onSubmit=\"if([PAGE_VALIDATION]){return submit_form();}else{return false;} \">
$sHiddenSrc
