...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if (!($sUseCurrentTable)) {
		$sOtDataTable = "otDataHistoryWorking";
		$sUserDataTable = "userDataHistoryWorking";
	} else {
		$sOtDataTable = "otData";
						...
						while ($oOffersRow = dbFetchObject($rOffersResult)) {
							...
							if ($sTempLeadsQuery != '') {
								...
								if ($sOtDataTable == 'otData') {
									...
									$sTempLeadsQuery = eregi_replace("AND postalVerified = 'V'","AND userDataHistoryWorking.postalVerified = 'V'",$sTempLeadsQuery);
								}
							}
